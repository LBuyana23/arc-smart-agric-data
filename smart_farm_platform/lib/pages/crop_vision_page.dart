...
class _CropVisionPageState extends State<CropVisionPage> with AutomaticKeepAliveClientMixin {
  ...
  void initState() {
    super.initState();
    _statusNotifier = api.serviceStatus;
        ...
        children: [
          ValueListenableBuilder<ServiceStatus>(
            ...
            builder: (context, status, _) {
              final shouldShow = status.state != BackendState.online || (status.latency != null && status.latency! > const Duration(seconds: 4));
              if (!shouldShow) return const SizedBox.shrink();
              return ServiceStatusBanner(
                status: status,
                onRetry: () async {
                  try {
                    await api.warmupBackend();
                    if (mounted) setState(() {});
                  } catch (_) {}
                },
              );
          ...
          LayoutBuilder(
            builder: (context, constraints) {
              ...
              return Row(
                children: [Expanded(child: heading)],
              );
            },
          ),
          SizedBox(height: 8),
          ...
          FutureBuilder<Map<String, dynamic>?>(
            ...
            builder: (context, snap) {
              ...
              return Column(
                children: map.entries.map((e) => _buildGroupSection(e.key, e.value)).toList(),
              );
            },
          ),
        ],
  ...
  Widget _buildGroupSection(String src, dynamic payload) {
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          PaginatedHistory(
            ...
            source: src,
            limit: 200,
            fetcher: (offset, limit) => api.fetchCropVisionDetections(source: src.toString(), offset: offset, limit: limit),
