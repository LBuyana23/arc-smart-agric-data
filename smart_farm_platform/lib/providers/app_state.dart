...
class AppState extends ChangeNotifier {
  ...
  final List<double> _liveData = List.generate(60, (i) => 20 + Random().nextDouble() * 10);
  ...
  AppState({bool startSimulations = true}) {
    if (startSimulations) {
      ...
      // When not simulating, start syncing real data from ApiService
      _api = ApiService();
  ...
  void _startDataSync() {
    ...
    Timer.periodic(Duration(minutes: 10), (_) async {
      try {
        weeklySoilMoistureChangePct = await _insights.calculateWeeklySoilMoistureChange();
        weeklyIrrigationVolume = await _insights.calculateWeeklyIrrigationVolume();
    ...
    Timer.periodic(Duration(minutes: 30), (_) async {
      try {
        ...
        if (wd != null) {
          ...
          threeDayForecast = wd.daily.take(3).map((d) => {'date': d.date.toIso8601String(), 'min': d.minTemp, 'max': d.maxTemp, 'code': d.weatherCode}).toList();
          notifyListeners();
        }
      } catch (_) {}
    });
  ...
  Future<void> setWeatherLocation(double lat, double lon) async {
    try {
      await _weather.saveLocation(lat, lon);
    ...
    try {
      ...
      if (ghRaw != null) {
        ...
        for (final entry in ghRaw.values) {
          if (entry is Map) {
            final t = entry['temperature'] ?? entry['temp'] ?? entry['TEMPERATURE'];
        ...
        if (temps.isNotEmpty) greenhouseAvgTemp = temps.reduce((a, b) => a + b) / temps.length;
        if (hums.isNotEmpty) greenhouseAvgHumidity = hums.reduce((a, b) => a + b) / hums.length;
      }
      ...
      if (irrRaw != null) {
        ...
        if (node is Map && node['data'] is Map && node['data']['items'] is List) {
          for (final it in node['data']['items']) if (it is Map) items.add(Map<String, dynamic>.from(it));
        }
        double sumFlow = 0.0;
        final pumps = <dynamic>{};
        int onCount = 0;
        for (final it in items) {
          final flow = double.tryParse(it['flowRate']?.toString() ?? it['flow_rate']?.toString() ?? '0') ?? 0.0;
          sumFlow += flow;
          final pumpState = (it['pumpState'] ?? it['pump_status'] ?? '').toString();
          if (pumpState.toUpperCase() == 'ON') onCount += 1;
          final id = it['device_id'] ?? it['id'] ?? it['DEVICE_ID'];
          if (id != null) pumps.add(id);
        }
      ...
      if (soilRaw != null) {
        ...
        for (final entry in soilRaw.values) {
          if (entry is Map && entry['data'] is Map && entry['data']['items'] is List) {
            for (final it in entry['data']['items']) {
              if (it is Map) {
                final m = double.tryParse(it['moisture']?.toString() ?? it['moisture_pct']?.toString() ?? '');
                final p = double.tryParse(it['ph']?.toString() ?? it['pH']?.toString() ?? '');
                if (m != null) moist.add(m);
                if (p != null) phs.add(p);
              }
            }
          }
        }
        if (moist.isNotEmpty) soilAvgMoisture = moist.reduce((a, b) => a + b) / moist.length;
        if (phs.isNotEmpty) soilAvgPh = phs.reduce((a, b) => a + b) / phs.length;
      }
          ...
          if (entry is Map && entry['data'] is Map && entry['data']['items'] is List) {
            for (final it in entry['data']['items']) {
              if (it is Map) {
                ...
                final conf = double.tryParse(it['confidence']?.toString() ?? it['score']?.toString() ?? '0') ?? 0.0;
                if (disease.toLowerCase() != 'healthy' && disease.toLowerCase() != 'unknown') {
                  threats += 1;
                }
                if (conf > topConf) {
                  topConf = conf;
